...
class ApplyView(APIView):
    ...
    def get(self, request):
        ...
        apply_status = Status.objects.get(status=apply_status).id
        query = Q(status=apply_status) & Q(user_id=user_id)
        
        data = JobPostActivity.objects.filter(query)
        apply_serialzer = JobPostActivitySerializer(data, many=True)
        return Response(apply_serialzer.data, status=status.HTTP_200_OK)
